from salus.exceptions import ConflictError, NotFoundError
from salus.models import MetricType
from salus.repositories.unit_of_work import IUnitOfWork
from salus.schemas import MetricTypeCreate


class MetricTypeService:
    def __init__(self, uow: IUnitOfWork) -> None:
        self.uow = uow

    def get(self, metric_type_id: str, user_id: str) -> MetricType:
        metric_type = self.uow.metric_types.get_by_id(metric_type_id)
        if metric_type is None:
            raise NotFoundError(f"MetricType {metric_type_id} not found")
        if metric_type.user_id != user_id:
            raise NotFoundError(f"MetricType {metric_type_id} not found")
        return metric_type

    def find_all(self, user_id: str) -> list[MetricType]:
        return self.uow.metric_types.find_all(user_id)

    def create(self, data: MetricTypeCreate, user_id: str) -> MetricType:
        existing = self.uow.metric_types.find_by_name_and_user(data.name, user_id)
        if existing is not None:
            raise ConflictError(f"MetricType '{data.name}' already exists")
        metric_type = MetricType(**data.model_dump(), user_id=user_id)
        return self.uow.metric_types.create(metric_type)

    def update(
        self, metric_type_id: str, user_id: str, data: MetricTypeCreate
    ) -> MetricType:
        metric_type = self.get(metric_type_id, user_id)
        existing = self.uow.metric_types.find_by_name_and_user(data.name, user_id)
        if existing is not None and existing.id != metric_type_id:
            raise ConflictError(f"MetricType '{data.name}' already exists")
        for field, value in data.model_dump().items():
            setattr(metric_type, field, value)
        return self.uow.metric_types.update(metric_type)

    def delete(self, metric_type_id: str, user_id: str) -> None:
        metric_type = self.get(metric_type_id, user_id)
        if metric_type.is_system:
            raise ConflictError("Cannot delete system metric types")
        self.uow.metric_types.delete(metric_type)

    def reorder(self, user_id: str, ordered_ids: list[str]) -> None:
        self.uow.metric_types.reorder(user_id, ordered_ids)
