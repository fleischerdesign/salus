...
class ActivityAnalysisService:
    ...
    def steps_trend(self, days: int = 7) -> list[StepDay]:
        result: list[StepDay] = []
        today = datetime.today()
        for i in range(days):
            date = (today - timedelta(days=i)).strftime("%Y-%m-%d")
            day_start, day_end = samsung_day_boundary(date)
            records = self._repo.find_all(data_types=["steps"])
            count = 0
            for rec in records:
                if rec.end_time and day_start <= rec.end_time.strftime("%Y-%m-%dT%H:%M:%S") < day_end:
                    if rec.value_numeric is not None:
                        ...
                        if c > count:
                            count = c
            result.append(StepDay(date=date, count=count))
        result.sort(key=lambda s: s.date)
        return result

    def heart_rate_summary(self, date_str: str | None = None) -> HRSummary | None:
        if date_str is None:
        ...
        records = self._repo.find_all(
            data_types=["heart_rate"], since=since, until=until
        )
        bpms: list[float] = []
        for rec in records:
    ...
    def exercise_history(self, days: int = 30, limit: int = 10) -> list[ExerciseSession]:
        since = datetime.today() - timedelta(days=days)
        records = self._repo.find_all(
            data_types=["exercise"], since=since, limit=limit
        )
