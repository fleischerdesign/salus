...
class AutoregulationService:
    ...
    def calculate_recovery_score(self, user_id: int, date_str: Optional[str] = None) -> tuple[float, float, float, float]:
        """
        ...
        if sleep_trend:
            baseline_sleep = sum(s.duration_hours for s in sleep_trend) / len(sleep_trend)
        else:
            baseline_sleep = 8.0
        ...
        for i in range(1, 8):
            ...
            sum_hr = self.activity_svc.heart_rate_summary(user_id=user_id, date_str=day_str)
            if sum_hr and sum_hr.resting_bpm:
                rhrs.append(sum_hr.resting_bpm)
        baseline_rhr = sum(rhrs) / len(rhrs) if rhrs else 60.0
        
        today_hr = self.activity_svc.heart_rate_summary(user_id=user_id, date_str=date_str)
        ...
        steps_trend = self.activity_svc.steps_trend(days=8, user_id=user_id, date=date_str)
        if len(steps_trend) >= 2:
            yesterday_steps = steps_trend[-2].count
            baseline_steps = sum(s.count for s in steps_trend[:-1]) / (len(steps_trend) - 1)
            if baseline_steps <= 0:
                baseline_steps = 10000.0
                
            if yesterday_steps > baseline_steps:
                steps_score = 80.0 - ((yesterday_steps - baseline_steps) / baseline_steps) * 40.0
            else:
                steps_score = 80.0 + ((baseline_steps - yesterday_steps) / baseline_steps) * 10.0
        else:
            steps_score = 80.0
    ...
    def get_autoregulated_targets(
        ...
        """
        overall, sleep_score, rhr_score, steps_score = self.calculate_recovery_score(user_id, date_str)
        
        results = []
        lower_body_muscles = {"quadriceps", "hamstrings", "gluteus_maximus", "gastrocnemius", "soleus"}
        ...
        for plan_ex, ex in exercises_with_targets:
            ...
            if overall > 85.0:
                ...
                weight_mult = 1.05
                rpe_adjust = 0.5
                reasons.append(f"Excellent recovery ({overall:.0f}% score). Suggesting peak weights/volume.")
            elif overall < 40.0:
                sets_adjust = -1
                weight_mult = 0.85
                rpe_adjust = -2.0
                reasons.append(f"Critical fatigue ({overall:.0f}% score). Suggesting deload.")
            elif overall < 60.0:
                sets_adjust = -1
                weight_mult = 0.92
                rpe_adjust = -1.0
                reasons.append(f"Mild fatigue ({overall:.0f}% score). Suggesting conservative load.")
            else:
            ...
            if steps_score < 75.0:
                ...
                primary = {m.strip().lower() for m in ex.primary_muscles.split(",") if m.strip()}
                secondary = {m.strip().lower() for m in (ex.secondary_muscles or "").split(",") if m.strip()}
