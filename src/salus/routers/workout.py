...
async def workouts_create_plan_post(
    ...
    for idx in range(5):
        ...
        if ex_id_str:
            ...
            exercises.append(WorkoutPlanExerciseCreate(
                exercise_id=ex_id,
                sequence=idx,
                target_sets=sets,
                target_reps=reps,
...
@router.post("/api/v1/workouts/exercises", response_model=ExerciseResponse, status_code=status.HTTP_201_CREATED)
...
@router.delete("/api/v1/workouts/exercises/{exercise_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.post("/api/v1/workouts/plans", response_model=WorkoutPlanResponse, status_code=status.HTTP_201_CREATED)
...
@router.delete("/api/v1/workouts/plans/{plan_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def get_plan_targets(
    ...
    return service.get_session_targets(user_id=uid(current_user), plan_id=plan_id, date_str=date)
...
@router.get("/api/v1/workouts/sessions/active", response_model=Optional[WorkoutSessionResponse])
...
async def log_set(
    ...
    return service.log_set(user_id=uid(current_user), session_id=session_id, entry=entry)
...
async def complete_session(
    ...
    return service.complete_session(user_id=uid(current_user), session_id=session_id, notes=notes)


@router.get("/api/v1/workouts/sessions/recent", response_model=list[WorkoutSessionResponse])
