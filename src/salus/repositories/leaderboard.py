...
class LeaderboardGroupRepository(
    Repository[LeaderboardGroup], ILeaderboardGroupRepository
):
    ...
    def find_by_invite_code(self, code: str) -> LeaderboardGroup | None:
        stmt = select(LeaderboardGroup).where(LeaderboardGroup.invite_code == code)
...
class LeaderboardMemberRepository(
    Repository[LeaderboardMember], ILeaderboardMemberRepository
):
