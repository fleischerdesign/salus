...
<div class="flex flex-col gap-1">
  <div class="flex items-baseline gap-1.5">
    <span class="text-3xl font-black tabular-nums" style="color: {color}">{displayValue}</span>
    {#if unit}
  ...
  {#if delta}
    ...
    <div class="flex items-center gap-1" style="color: {dc}">
      ...
      <span class="text-xs font-bold tabular-nums">{delta.value}</span>
    </div>
  {/if}

  {#if subLabel}
    <span class="text-xs text-[var(--text-soft)]">{subLabel}</span>
  {/if}
</div>
