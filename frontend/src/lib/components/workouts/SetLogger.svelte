...
>
  ...
  <div class="flex items-center gap-1.5">
    ...
    <div class="{stepperClass} {isRpe10 && isOn ? 'border-error-300' : ''}">
      <button
        type="button"
        class={btnClass}
        onclick={() => adjust('rpe', -0.5)}
        disabled={isOn}
        aria-label="Decrease RPE">−</button
      >
      <input
        type="number"
        value={rpe}
      ...
      <button
        ...
        onclick={() => adjust('rpe', 0.5)}
        disabled={isOn}
        aria-label="Increase RPE">+</button
      >
    </div>
  </div>
  ...
  <!-- Est. 1RM -->
  <span
    class="flex items-center gap-1 text-xs font-semibold tabular-nums {isPR
      ? 'text-warning-600'
      : 'text-surface-400'}"
