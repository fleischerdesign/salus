...
>
  ...
  {#if plans.length === 0}
    <div class="space-y-1 py-6 text-center text-xs text-[var(--text-muted)]">
      <p class="font-semibold text-[var(--text-main)]">Noch kein Trainingssplit definiert</p>
  ...
  {:else}
    <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-7">
      {#each daysOfWeek as day, idx}
        ...
        <div
          class="flex flex-col justify-between rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface-50)] p-3"
        >
          <div class="mb-1 flex items-center justify-between">
            <span
              class="rounded bg-[var(--bg-surface-0)] px-1.5 py-0.5 font-mono text-xs font-bold text-[var(--text-main)]"
            >
              {day}
            </span>
          </div>
          <div class="mt-1 truncate text-xs font-bold text-[var(--text-main)]">
            {plan?.name || 'Regeneration'}
          </div>
          <p class="mt-0.5 truncate text-[0.6875rem] text-[var(--text-muted)]">
