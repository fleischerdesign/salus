...
<div class="space-y-6">
  ...
  <!-- Insights List -->
  {#if loading}
    <div class="py-12 text-center text-sm text-text-muted">Erkenntnisse werden berechnet...</div>
  {:else if insights.length === 0}
    ...
    >
      ...
      <h3 class="text-base font-bold text-text-main">Noch keine Coaching-Impulse generiert</h3>
      <p class="mx-auto max-w-md text-xs text-text-muted">
  ...
  {:else}
    <div class="space-y-4">
      {#each insights as ins (ins.id)}
        <div class="space-y-3 rounded-2xl border border-border-subtle bg-surface-0 p-6 shadow-card">
          ...
          <p class="text-xs leading-relaxed text-text-muted">{ins.content}</p>
        </div>
      {/each}
    </div>
  {/if}
