...
<div class="space-y-6">
  ...
  <!-- Habit Grid -->
  {#if loading}
    <div class="py-12 text-center text-sm text-text-muted">Gewohnheiten werden geladen...</div>
  {:else if habits.length === 0}
    ...
    >
      ...
      <div>
        <h3 class="text-base font-bold text-text-main">Noch keine Gewohnheiten angelegt</h3>
      ...
      {#each habits as h (h.id)}
        <div class="rounded-2xl border border-border-subtle bg-surface-0 p-5 shadow-card">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="flex items-center gap-2">
                <span class="text-base font-bold text-text-main">{h.title}</span>
                <Badge variant="default" class="text-[0.625rem]">{h.category}</Badge>
              </div>
              <p class="mt-0.5 text-xs text-text-muted">Frequenz: {h.frequency}</p>
            </div>
  ...
  >
    ...
    >
      <div>
        <label for="habit-name" class="mb-1 block text-xs font-bold text-text-main"
          >Name der Gewohnheit</label
        >
        <Input id="habit-name" bind:value={newName} placeholder="z. B. 3 Liter Wasser trinken" />
      </div>
      <div>
        <label for="habit-cat" class="mb-1 block text-xs font-bold text-text-main"
