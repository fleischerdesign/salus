    ...
    <div class="space-y-6">
      ...
      >
        <div class="flex items-center gap-3.5">
          <div
            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-primary-soft text-primary"
          >
  ...
  {:else if activeTab === 'sessions'}
    {#if pastSessions.length === 0}
      ...
      >
        ...
        <h3 class="text-base font-bold text-text-main">Noch keine absolvierten Workouts</h3>
        ...
        {#each pastSessions as s}
          <div class="space-y-3 rounded-3xl border border-border-subtle bg-surface-0 p-5 shadow-xs">
            <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 class="text-base font-extrabold text-text-main">{s.planName}</h3>
                <p class="mt-0.5 text-xs text-text-muted">
              ...
              <div class="flex items-center gap-4">
                <div>
                  <span class="block text-right text-xs font-semibold text-text-muted"
                    >Volumen-Tonnage</span
                  >
                  <span class="block text-right text-base font-extrabold text-activity tabular-nums"
                    >{s.tonnage}</span
                  >
                </div>
  ...
  {:else if activeTab === 'exercises'}
    <div class="space-y-6">
      ...
      <div class="space-y-4 rounded-3xl border border-border-subtle bg-surface-0 p-5 shadow-xs">
        <div
          class="flex flex-wrap items-center justify-between gap-3 border-b border-border-subtle/60 pb-2"
        >
          <div>
        ...
        {#if filteredExercises.length === 0}
          <div class="space-y-2 py-10 text-center text-xs text-text-muted">
            <Icon name="search" size="lg" class="mx-auto text-text-muted opacity-60" />
            <p class="text-xs font-bold text-text-main">Keine passenden Übungen gefunden</p>
            <p class="mx-auto max-w-sm text-[0.6875rem]">
          ...
          <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
            {#each filteredExercises as ex}
              <a
                ...
                class="group flex flex-col justify-between space-y-3 rounded-2xl border border-border-subtle bg-surface-50 p-4 transition-all hover:border-border-strong hover:bg-surface-0 hover:shadow-xs"
              >
                <div>
                  <div class="mb-1.5 flex items-start justify-between gap-2">
                    <h3
                      class="text-sm leading-snug font-extrabold text-text-main transition-colors group-hover:text-primary"
                    >
                      {ex.name}
                    </h3>
                    <Badge
                  ...
                  {#if ex.description || ex.instructions}
                    <p class="mt-2 line-clamp-2 text-[0.6875rem] leading-relaxed text-text-muted">
                ...
                <div
                  class="flex items-center justify-between border-t border-border-subtle pt-2.5 text-[11px] text-text-muted"
                >
                  <div class="flex items-center gap-1.5">
                  ...
                  <span class="font-mono text-[10px]">
                    {ex.suggested_rest_seconds || 90}s Pause
                  </span>
                </div>
              </a>
            {/each}
