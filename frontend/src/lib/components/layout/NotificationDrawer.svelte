...
{#if open}
  ...
  <div
    class="glass-panel fixed top-0 right-0 bottom-0 z-61 flex w-full max-w-md flex-col border-l border-border-subtle text-text-main shadow-2xl"
  ...
  >
    ...
    <div class="flex items-center justify-between border-b border-border-subtle px-5 py-4">
      <div class="flex items-center gap-2.5">
        <div
          class="flex h-8 w-8 items-center justify-center rounded-full bg-primary-soft font-bold text-primary"
        >
          <Icon name="notifications" size="sm" />
        </div>
        <div>
          <h2 class="text-sm font-extrabold text-text-main">Mitteilungen &amp; Impulse</h2>
      ...
      <div class="flex items-center gap-1.5">
        {#if unreadCount > 0}
          <button
            ...
            class="cursor-pointer rounded-lg px-2 py-1 text-[0.6875rem] font-bold text-primary transition-colors hover:underline"
          >
            Alle gelesen
          </button>
        {/if}
    ...
    <!-- Filter Pills -->
    <div
      class="no-scrollbar flex items-center gap-1.5 overflow-x-auto border-b border-border-subtle bg-surface-50/40 px-5 py-2.5"
    >
      <button
        type="button"
      ...
      <button
        ...
        class="cursor-pointer rounded-full px-2.5 py-1 text-xs font-semibold whitespace-nowrap transition-colors {filterType ===
        'circadian'
          ? 'bg-circadian font-bold text-white'
          : 'text-text-muted hover:bg-surface-100'}"
      >
        Zirkadian
      </button>
    ...
    <div class="flex-1 space-y-2.5 overflow-y-auto p-4">
      {#if filteredList.length === 0}
        <div
          class="flex h-64 flex-col items-center justify-center space-y-2 text-center text-text-muted"
        >
          <Icon name="check_circle" size="lg" class="text-emerald-500 opacity-60" />
          <p class="text-xs font-bold">Alles erledigt!</p>
          <p class="text-[0.6875rem] text-text-soft">
        ...
        {#each filteredList as item}
          ...
          <div
            class="group relative flex flex-col gap-1.5 rounded-2xl border p-3.5 transition-all {item.read
              ? 'border-border-subtle bg-surface-50/40 text-text-muted'
              : 'border-primary/30 bg-surface-0 text-text-main shadow-xs'}"
          >
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="shrink-0 text-[0.625rem] font-medium text-text-soft">{item.time}</span>
            </div>

            <p class="pl-8 text-xs leading-relaxed text-text-muted">
              {item.desc}
            </p>

            <div
              class="mt-1 flex items-center justify-between border-t border-border-subtle/50 pt-1 pl-8"
            >
              <button
                type="button"
