<script lang="ts">
  ...
  const { value: notifications = [] } = useQuery(() =>
    db.notification
      ...
      .toArray()
      .then((arr) => arr.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())),
  );
  ...
  async function markAllRead() {
    ...
    await db.notification
      .filter((n) => !n.is_read && !n.deleted_at)
      .modify({ is_read: true });
  }

  let menuClass = $derived('absolute right-0 top-full mt-2 w-80 rounded-lg border border-surface-200 bg-surface-0 py-1 shadow-lg z-dropdown transition-all duration-150 ' +
...
<div class="relative" onmouseenter={show} onmouseleave={hide} role="presentation">
  ...
  >
    ...
    {#if unreadCount > 0}
      <span class="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-error-500 px-1 text-[10px] font-bold text-white">
  ...
  <div class={menuClass}>
    <div class="flex items-center justify-between border-b border-surface-200 px-4 py-2">
      ...
      {#if unreadCount > 0}
        <button type="button" class="text-xs font-medium text-primary-600 hover:text-primary-700" onclick={markAllRead}>
          Mark all read
        </button>
      {/if}
    </div>
    <div class="max-h-80 overflow-auto">
      ...
      {:else}
        {#each notifications as n}
          <div class="border-b border-surface-100 px-4 py-3 transition-colors duration-150 hover:bg-surface-50 {n.is_read ? 'opacity-60' : ''}">
            <div class="flex items-start gap-2">
              ...
              <div class="min-w-0 flex-1">
                ...
                {#if n.created_at}
                  <p class="mt-1 text-[11px] text-surface-400">{new Date(n.created_at).toLocaleString()}</p>
                {/if}
              </div>
            </div>
          </div>
