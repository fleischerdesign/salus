      ...
      <div class="h-2 flex-1 overflow-hidden rounded-full bg-surface-100">
        <div
          ...
          style="width: {totalMacroCals > 0
            ? Math.min((proteinCals / totalMacroCals) * 100, 100)
            : 0}%"
        ></div>
      </div>
      <span class="w-16 text-right text-xs font-medium text-surface-600"
        >{Math.round(totalProtein)}g</span
      >
    </div>
    <div class="flex items-center gap-2">
