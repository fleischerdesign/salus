...
<div class="rounded-2xl border border-border-subtle bg-surface-0 p-5 shadow-card">
  <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
    <div>
      <div class="flex items-center gap-1.5 text-sm font-bold text-text-main">
        ...
        <h3 class="text-sm font-bold text-text-main">Rezept-Portionen-Skalierer</h3>
      </div>
      <p class="mt-0.5 text-xs text-text-muted">Dynamische Portions- und Nährwertberechnung</p>
    </div>

    <!-- Portion Controls -->
    <div class="flex items-center gap-2 rounded-lg border border-border-subtle bg-surface-50 p-1">
      <button
        type="button"
        class="h-7 w-7 cursor-pointer rounded bg-surface-0 text-xs font-bold hover:bg-surface-100"
      ...
      <tbody class="divide-y divide-border-subtle">
        {#each scaledIngredients as ing}
          <tr>
            ...
            <td class="px-3 py-2.5 font-mono font-bold text-vital">{ing.protein}g</td>
          </tr>
        {/each}
      </tbody>
    </table>
  ...
  >
    ...
    <div class="flex items-center gap-4 font-mono font-bold">
      ...
      <span class="text-vital">{totalProtein}g Protein</span>
    </div>
  </div>
</div>
