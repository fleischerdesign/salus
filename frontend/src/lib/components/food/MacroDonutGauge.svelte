...
>
  ...
  <div
    class="grid grid-cols-1 items-center gap-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface-50)] p-4 md:grid-cols-12"
  >
    <!-- Donut Hero -->
    <div class="relative mx-auto flex h-44 w-44 items-center justify-center md:col-span-5">
      ...
      <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
        <span class="text-3xl font-extrabold text-[var(--text-main)] tabular-nums"
          >{calories.current}</span
        >
        <span class="text-[0.625rem] font-bold text-[var(--text-muted)] uppercase"
          >kcal gegessen</span
        >
        <span class="mt-0.5 text-xs font-bold text-[var(--color-activity)] tabular-nums"
