...
<div class="space-y-1 text-xs {extraClass}" {style}>
  ...
  <div
    class="relative flex items-center rounded-xl border bg-surface-0 transition-all duration-150 {isFocused
      ? 'border-primary shadow-xs ring-2 ring-primary/15'
      : error
        ? 'border-rose-500 ring-2 ring-rose-500/15'
        : 'border-border-subtle hover:border-border-strong'} {disabled
      ? 'cursor-not-allowed bg-surface-100 opacity-60'
      : ''}"
  >
    {#if icon}
    ...
    >
      {#each options as opt}
        <option
          ...
          class="bg-surface-0 text-text-main"
        >
          {opt.label}
    ...
    <div
      class="pointer-events-none absolute top-1/2 right-3.5 flex -translate-y-1/2 items-center justify-center text-text-muted"
    >
