<script lang="ts">
  ...
  let { label, name, value = '', onchange, required = false, error, hint, disabled = false, class: extraClass = '' }: Props = $props();
...
<div class="input {extraClass}">
  {#if label}
    <label for="{name ?? 'datepicker'}-input" class="mb-1 block text-[13px] font-semibold tracking-[0.05em] text-surface-900">
      {label}{#if required}<span class="text-error-500"> *</span>{/if}
    </label>
  {/if}
  <input id="{name ?? 'datepicker'}-input" type="date" {name} value={value} oninput={handleInput} {disabled} {required} class={inputClass} />
  {#if error}
