...
>
  ...
  <div class="space-y-4 text-xs">
    ...
    <div
      class="space-y-2 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface-50)] p-3"
    >
      <div class="flex items-center justify-between">
        <span class="font-bold">Zusätzlicher Schlaf pro Nacht</span>
        <span class="font-bold text-[var(--color-primary)]"
          >+{sleepDeltaHours.toFixed(1)} Std / Nacht</span
        >
      </div>
    ...
    >
      ...
      <div class="flex justify-between text-[0.6875rem] text-[var(--text-muted)]">
        ...
        <strong class="font-bold text-[var(--color-primary)]"
          >{projected1RM} kg (in 6 Wochen)</strong
        >
      </div>
    </div>
  </div>
  ...
  <div
    class="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface-50)] p-2.5 text-[0.6875rem] text-[var(--text-muted)]"
  >
    <strong class="text-[var(--text-main)]">Evidenz-Basis:</strong> Berechnet aus deinen n = 90 persönlichen
    Längsschnitt-Tagen kombiniert mit Meta-Analysen der Cochrane Database und ESC Guidelines.
  </div>
</div>
