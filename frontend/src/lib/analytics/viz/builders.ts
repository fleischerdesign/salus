...
function parseSleepJson(json: string | null): { deep: number; rem: number; light: number; awake: number; total_seconds: number } | null {
  if (!json) return null;
  try {
    ...
    return { deep: deep / 3600, rem: rem / 3600, light: light / 3600, awake: awake / 3600, total_seconds: total };
  } catch {
    return null;
  }
}
...
function findGoal(goals: Goal[], sourceDataType: string, metricTypeId: number): Goal | null {
  return goals.find((g) => g.deleted_at == null && g.is_active && g.metric_type_id === metricTypeId) ?? null;
}
...
export function buildStepsViz(ctx: VizContext): WidgetViz {
  ...
  if (todaySteps == null) {
    return { type: 'progress', title: 'Steps', value: '—', unit: 'steps', empty: true, empty_text: 'No step data yet.' };
  }
  ...
  const viz: WidgetViz = {
    ...
    goal_percent: goal ? Math.min(100, Math.floor((todaySteps / goal.target_value) * 100)) : undefined,
...
export function buildHeartRateViz(ctx: VizContext): WidgetViz {
  ...
  if (dayM.length === 0) {
    return { type: 'pills', title: 'Heart Rate', value: '—', unit: 'bpm', empty: true, empty_text: 'No heart rate data.' };
  }
  ...
  const yestM = ctx.allMeasurements.filter((m) => m.start_time.startsWith(yesterday(ctx.date)) && !m.deleted_at);
...
export function buildSleepViz(ctx: VizContext): WidgetViz {
  ...
  if (!sleep) {
    return { type: 'bar', title: 'Sleep', value: '—', unit: 'h', empty: true, empty_text: 'No sleep data yet.' };
  }
...
export function buildWeightViz(ctx: VizContext): WidgetViz {
  ...
  if (w == null) {
    return { type: 'number', title: 'Weight', value: '—', unit: 'kg', empty: true, empty_text: 'No weight data yet.' };
  }
...
export function buildNutritionViz(ctx: VizContext): WidgetViz {
  ...
  if (todayN.length === 0) {
    return { type: 'bar', title: 'Nutrition', value: '—', unit: 'kcal', empty: true, empty_text: 'No nutrition data.' };
  }
  let proteinG = 0;
  let carbsG = 0;
  let fatG = 0;
...
export function buildExerciseViz(ctx: VizContext): WidgetViz {
  ...
  if (totalMin <= 0) {
    return { type: 'number', title: 'Exercise', value: '—', unit: 'min', empty: true, empty_text: 'No exercise data.' };
  }
  return {
    type: 'number',
    title: 'Exercise',
...
export function buildGenericViz(ctx: VizContext): WidgetViz {
  const latestM = ctx.allMeasurements
    .filter((m) => m.metric_type_id === ctx.metric.id && !m.deleted_at && m.start_time.startsWith(ctx.date))
  ...
  if (latestM.value_numeric != null) {
    value = latestM.value_numeric % 1 === 0 ? latestM.value_numeric.toFixed(0) : latestM.value_numeric.toFixed(1);
