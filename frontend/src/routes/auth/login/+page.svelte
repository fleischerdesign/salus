  ...
  <div class="w-full max-w-md space-y-6">
    ...
    >
      ...
      {#if Capacitor.isNativePlatform()}
        <div class="space-y-2.5 rounded-2xl border border-border-subtle bg-surface-50 p-3.5">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2 overflow-hidden">
              <Icon name="dns" size="sm" class="shrink-0 text-text-muted" />
              <span class="truncate text-xs font-semibold text-text-main">
                {getApiBaseUrl() || 'Kein Server verbunden'}
              </span>
            </div>
            <button
              type="button"
              class="shrink-0 cursor-pointer text-xs font-bold text-primary hover:underline"
          ...
          {#if showServerConfig}
            <div class="space-y-2 border-t border-border-subtle pt-2">
              ...
              <button
                ...
                class="w-full cursor-pointer rounded-xl border border-border-subtle bg-surface-0 py-2 text-xs font-bold text-text-main transition-all hover:bg-surface-50 disabled:opacity-50"
              >
      ...
      {#if authConfig.oidcProviders.length > 0}
        <div class="space-y-3 pt-2">
          <div class="relative flex items-center justify-center">
            ...
            <span
              class="absolute bg-glass-dock px-3 text-[0.6875rem] font-bold tracking-wider text-text-muted uppercase"
            >
              oder
            </span>
          </div>
          ...
          <div class="flex flex-col gap-2 pt-2">
            {#each authConfig.oidcProviders as provider}
              ...
              <button
                ...
                class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-border-subtle bg-surface-0 py-2.5 text-xs font-bold text-text-main shadow-2xs transition-all hover:bg-surface-50"
              >
      ...
      <div class="border-t border-border-subtle pt-4">
        <div class="space-y-3 rounded-2xl border border-border-subtle bg-surface-50 p-4">
          <div class="flex items-center gap-2">
            ...
            <div>
              <span class="block text-xs font-bold text-text-main">Ohne Server nutzen</span>
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              type="button"
              onclick={startLocal}
              class="h-10 shrink-0 cursor-pointer rounded-xl border border-border-subtle bg-surface-0 px-4 text-xs font-bold text-text-main shadow-2xs transition-all hover:bg-surface-100"
            >
              Starten
            </button>
          </div>
      ...
      <div class="pt-2 text-center">
        <p class="text-xs text-text-muted">
          Noch kein Konto?
          <a href="/auth/register" class="ml-1 font-bold text-primary hover:underline">
            Konto erstellen
          </a>
        </p>
      </div>
