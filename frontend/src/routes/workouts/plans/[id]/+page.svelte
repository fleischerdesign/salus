...
import { mutate, nextTempId } from '$lib/db/mutate';
  ...
  let planExercises = liveQuery(() =>
    db.workout_plan_exercise.toArray().then((arr) =>
      arr.filter((pe) => pe.plan_id === planId && !pe.deleted_at).sort((a, b) => a.sequence - b.sequence),
    ),
  );
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <div class="mt-2 flex items-center gap-1.5">
          <Badge variant={$plan.autoreg_mode === 'disabled' ? 'default' : 'primary'} class="capitalize">
            {$plan.autoreg_mode}
          </Badge>
    ...
    <div class="grid gap-6 lg:grid-cols-[2fr_1fr]">
      <div class="space-y-3">
        ...
        {#if $planExercises.length === 0}
          <EmptyState title="No exercises" description="No exercises in this plan yet." icon="exercise" />
        {:else}
          <Card padding={false}>
            <div class="divide-y divide-surface-100">
              {#each $planExercises as pe (pe.id)}
                ...
                <ListItem hoverable>
                  {#snippet children()}
                    <a href="/workouts/exercises/{pe.exercise_id}" class="flex min-w-0 flex-1 items-center justify-between gap-3 no-underline">
                      <div class="min-w-0">
                        <p class="truncate text-sm font-medium text-surface-900">{ex?.name ?? 'Unknown'}</p>
                        <p class="mt-0.5 text-xs capitalize text-surface-400">
                          {ex?.equipment ?? ''} · {ex?.primary_muscles ?? ''}
                        </p>
                      </div>
                      <div class="shrink-0 text-right">
                        <p class="text-sm font-semibold text-surface-700">{pe.target_sets}×{pe.target_reps}</p>
      ...
      <div class="space-y-3">
        ...
        {#if !$sessions || $sessions.length === 0}
          <Card><p class="text-sm text-surface-400">No completed sessions for this plan yet.</p></Card>
        {:else}
          <Card padding={false}>
            <div class="divide-y divide-surface-100">
              {#each $sessions.slice(0, 10) as sess (sess.id)}
                <ListItem hoverable>
                  {#snippet children()}
                    <a href="/workouts/sessions/{sess.id}" class="flex min-w-0 flex-1 items-center justify-between gap-3 no-underline">
                      <div class="min-w-0">
                        <p class="truncate text-sm font-medium text-surface-700">
                          {new Date(sess.completed_at ?? sess.started_at).toLocaleDateString()}
                        </p>
                        <p class="text-xs text-surface-400">
                          {formatDuration(sess.started_at, sess.completed_at)} · {sessionVolume(sess.id).toFixed(0)} kg
                        </p>
                      </div>
