<script lang="ts">
  ...
  let logs = liveQuery(() =>
    db.workout_log_entry.toArray().then((arr) =>
      arr.filter((l) => l.exercise_id === exerciseId && !l.deleted_at),
    ),
  );
  ...
  let historyRows = $derived.by<HistoryRow[]>(() => {
    return ($logs ?? [])
      ...
      .sort((a, b) => {
        const da = new Date(($sessions?.get(a.session_id)?.completed_at ?? $sessions?.get(a.session_id)?.started_at ?? '')).getTime();
        const db = new Date(($sessions?.get(b.session_id)?.completed_at ?? $sessions?.get(b.session_id)?.started_at ?? '')).getTime();
        return db - da;
      })
      .slice(0, 200)
      .map((l) => {
  ...
  let prMaxWeight = $derived(
    Math.max(0, ...($logs ?? []).map((l) => l.weight ?? 0)),
  );
  ...
  let totalReps = $derived(
    ($logs ?? []).reduce((sum, l) => sum + (l.reps ?? 0), 0),
  );
...
<svelte:head
  ><title>Salus — {$exercise?.name ?? 'Exercise'}</title></svelte:head
>
    ...
    <div class="grid gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <Card padding={false}>
          ...
          <div class="p-6">
            ...
            {#if instructions.length > 0}
              <ol
                class="list-decimal space-y-2 pl-5 text-sm leading-relaxed text-surface-600"
              >
            ...
            {:else if !$exercise.description}
              <p class="text-sm text-surface-400">
                No detailed instructions recorded.
              </p>
            {/if}
      ...
      <div class="space-y-4">
        <Card padding={false}>
          ...
          {/snippet}
          <div
            class="flex flex-wrap items-center gap-x-6 gap-y-4 p-6"
          >
            <Stat
