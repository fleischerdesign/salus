    ...
    >
      ...
      {#snippet actions()}
        <div class="flex h-full items-stretch divide-x divide-surface-200 select-none">
          ...
          <button
            type="button"
            class="duration-micro text-error-600 hover:bg-error-50 hover:text-error-700 flex h-full items-center justify-center gap-2 px-6 text-sm font-semibold whitespace-nowrap transition-colors"
              ...
              {#each calendarCells as cell}
                <div
                  class="h-3 w-3 rounded-sm {cell.completed
                    ? 'bg-primary-500'
                    : 'bg-surface-100'} {cell.today ? 'ring-primary-300 ring-1 ring-inset' : ''}"
                  title={cell.date}
                ></div>
              {/each}
            </div>
