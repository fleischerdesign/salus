...
def test_delete_system_metric_rejected(authenticated_client):
    ...
    response = authenticated_client.delete(f"/entries/metric/{system_metric_id}")
    assert response.status_code == 409


def test_create_duplicate_rejected(authenticated_client):
    data = {"name": "DuplicateTest", "unit": "x", "data_type": "number", "color": "#000"}
    authenticated_client.post("/entries/metric", data=data, follow_redirects=True)
    response = authenticated_client.post("/entries/metric", data=data, follow_redirects=True)
    assert response.status_code == 409
...
def test_delete_custom_metric(authenticated_client):
    ...
    response = authenticated_client.delete(f"/entries/metric/{custom_metric_id}")
    assert response.status_code == 200
